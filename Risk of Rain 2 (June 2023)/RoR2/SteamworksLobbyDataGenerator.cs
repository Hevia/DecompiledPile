...
public static class SteamworksLobbyDataGenerator
{
	...
	{
		lobbyId = new CSteamID(Client.Instance.Lobby.CurrentLobby);
		...
		LobbyManager lobbyManager = PlatformSystems.lobbyManager;
		lobbyManager.onLobbyMemberDataUpdated = (Action<UserID>)Delegate.Combine(lobbyManager.onLobbyMemberDataUpdated, new Action<UserID>(SteamworksLobbyManagerOnOnLobbyMemberDataUpdated));
		LobbyManager lobbyManager2 = PlatformSystems.lobbyManager;
	...
	private static void OnLobbyOwnershipLost()
	{
		...
		NetworkManagerSystem.onStartClientGlobal -= NetworkManagerSystemOnOnStartClientGlobal;
		LobbyManager lobbyManager = PlatformSystems.lobbyManager;
		lobbyManager.onLobbyMemberDataUpdated = (Action<UserID>)Delegate.Remove(lobbyManager.onLobbyMemberDataUpdated, new Action<UserID>(SteamworksLobbyManagerOnOnLobbyMemberDataUpdated));
		LobbyManager lobbyManager2 = PlatformSystems.lobbyManager;
	...
	public static void RebuildLobbyData()
	{
		...
		try
		{
			...
			Lobby val = ((instance != null) ? instance.Lobby : null);
			if (val == null)
			{
				return;
			}
			LobbyData currentLobbyData = val.CurrentLobbyData;
			Dictionary<string, string> dictionary = ((currentLobbyData != null) ? currentLobbyData.GetAllData() : null);
			if (!val.IsOwner || dictionary == null || currentLobbyData == null)
			{
				return;
			}
			string value2 = null;
			...
			if (flag4 && !flag3 && flag)
			{
				...
				val2.Write(text);
				(PlatformSystems.lobbyManager as SteamworksLobbyManager).SendLobbyMessage(LobbyManager.LobbyMessageType.Password, val2);
			...
			{
				string value4 = null;
				if (dictionary == null || !dictionary.TryGetValue(item2.Key, out value4) || !item2.Value.Equals(value4, StringComparison.Ordinal))
				{
